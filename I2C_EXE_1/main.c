...
int main() {
    ...
    while (1) {
        ...
        if (pressed(SW0_PIN)) {
            set_led_state(&ls, 1);  // Left LED on, others off
            led_state_changed = true;
        }

        if (pressed(SW1_PIN)) {
            set_led_state(&ls, 1);  // Middle LED on, others off
            led_state_changed = true;
        }

        if (pressed(SW2_PIN)) {
            set_led_state(&ls, 1);  // Right LED on, others off
            led_state_changed = true;
        }
        if(led_state_changed){
            gpio_put(LED0_PIN, ls.state);
            gpio_put(LED1_PIN, ls.state);
            gpio_put(LED2_PIN, ls.state);
